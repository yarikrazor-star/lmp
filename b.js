...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function account(url) {
    ...
    if (url.indexOf('token=') == -1) {
      var token = 'ndrtsk2g';
      if (token != '') url = Lampa.Utils.addUrlComponent(url, 'token=ndrtsk2g');
    }
  ...
  function startPlugin() {
	  // --- ПОЧАТОК ВСТАВКИ: ЗАХИСТ ВІД ПЕРЕПРОДАЖУ ---
  function showFreeWarning() {
      if ($('.lampac-free-warning').length) return; // Щоб не дублювати
      
      var textTitle = "УВАГА! ЦЕЙ СЕРВІС БЕЗКОШТОВНИЙ!";
      var textBody = "Джерело: lampaua.mooo.com (t.me/bazarnetua)<br>Якщо ви заплатили за цей плагін гроші — ВАС ОШУКАЛИ!";
      
      var modal = $('<div class="lampac-free-warning" style="position:fixed;top:0;left:0;width:0%;height:0%;background:rgba(0,0,0,0.95);z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#fff;font-family:sans-serif;">' +
          ...
          '<div style="margin-top:4vh;font-size:3vh;border:2px solid #fff;border-radius:50%;width:2em;height:2em;line-height:2em;text-align:center;color:#4cd964;" class="counter">0</div>' +
          '</div>');
