(function () {
    ...
    var style = '<style id="bandera-badges-style">' +
        ...
        // --- DESKTOP / TV (Великі екрани) ---
        // Логіка "ВИЩЕ": position absolute + bottom 100% піднімає блок над батьківським елементом
        '@media (min-width: 501px) {' +
            '.ua-sites-container { position: absolute; right: 0; bottom: 100%; margin-bottom: 6px; justify-content: flex-end; }' +
        '}' +

        // --- MOBILE (Мобільні пристрої) ---
        // Логіка "ДРУГИЙ РЯДОК": position relative скидає накладання, блок стає знизу
        '@media (max-width: 500px) {' +
            '.ua-sites-container { position: relative; width: 100%; justify-content: flex-start; flex-wrap: wrap; margin-top: 10px; }' +
        '}' +

        // Стилі кнопок (бейджів)
    ...
    BanderaController.prototype.start = function() {
        ...
        // 2. Логіка вбудовування (як в оригіналі UaCineCardify)
        var torqCont = $('.quality-badges, .quality-badges-container', rateLine);

        if (torqCont.length) {
            // Є TorQual: вбудовуємось в нього.
            // Завдяки CSS (bottom: 100%) ми будемо візуально НАД ним.
            torqCont.css('position', 'relative').append(container);
        } else {
            // Немає TorQual: вбудовуємось в основний рядок.
            // Завдяки CSS (bottom: 100%) ми будемо візуально НАД рядком (на місці TorQual).
            rateLine.css('position', 'relative').append(container);
        }
    ...
    BanderaController.prototype.showSearchResults = function(sourceId, items) {
        ...
        Lampa.Select.show({
            ...
            items: menuItems,
            onSelect: function(a) {
                _this.loadStructure(sourceId, a.item);
            },
            onBack: function() {
            ...
            _this.showVoiceSelector(content.voices, function(voice) {
                _this.showSeasonSelector(voice.seasons, function(season) {
                    _this.showEpisodeSelector(season.episodes, function(episode) {
                     ...
                     _this.processContent(sourceId, content, mainTitle);
                });
            });
        } 
        // ФІЛЬМ
            ...
            if (content.voices && content.voices.length) {
                items = content.voices.map(function(v) {
                    ...
                    if (!ref && v.seasons && v.seasons[0] && v.seasons[0].episodes && v.seasons[0].episodes[0]) {
                        ref = v.seasons[0].episodes[0].ref;
                    }
                    return {
                        title: v.display_name || v.title || 'Варіант ' + (items.length + 1),
                        ref: ref
                    };
                ...
                items = content.streams.map(function(s) {
                    return {
                        title: s.title || s.quality || 'Основний',
                        ref: s.ref,
                        url: s.url
                    };
                });
    ...
    BanderaController.prototype.showVoiceSelector = function(voices, onSelect) {
        var menuItems = voices.map(function(v) {
            return {
                title: v.display_name || v.name || v.title || 'Озвучка',
                voice: v
            };
        });
        Lampa.Select.show({
            title: 'Виберіть озвучку',
            items: menuItems,
            onSelect: function(a) { onSelect(a.voice); },
            onBack: function() { Lampa.Controller.toggle('content'); }
        });
    };

    BanderaController.prototype.showSeasonSelector = function(seasons, onSelect, onBack) {
        var menuItems = seasons.map(function(s) {
            return {
        ...
        Lampa.Select.show({
            ...
            items: menuItems,
            onSelect: function(a) { onSelect(a.season); },
            onBack: onBack || function() { Lampa.Controller.toggle('content'); }
        });
    };

    BanderaController.prototype.showEpisodeSelector = function(episodes, onSelect, onBack) {
        var menuItems = episodes.map(function(e) {
            return {
                title: (e.number ? e.number + '. ' : '') + (e.title || 'Серія ' + e.number),
                episode: e
            };
        });
        Lampa.Select.show({
            ...
            items: menuItems,
            onSelect: function(a) { onSelect(a.episode); },
            onBack: onBack
        });
    };
    ...
    BanderaController.prototype.play = function(sourceId, ref, title) {
        ...
        Network.post(api_base + '/stream', {
            source: sourceId,
            ref: ref
        }, function(streamJson) {
            Lampa.Loading.stop();

            if (!streamJson || !streamJson.ok || !streamJson.streams || !streamJson.streams.length) {
                Lampa.Noty.show(Lampa.Lang.translate('online_nolink'));
                return;
            }
            ...
            if (Lampa.Favorite && typeof Lampa.Favorite.add === 'function') {
                Lampa.Favorite.add('history', _this.movie, 100);
            }
    ...
    BanderaController.prototype.playDirectUrl = function(url, title) {
        ...
        if (Lampa.Favorite && typeof Lampa.Favorite.add === 'function') {
            Lampa.Favorite.add('history', this.movie, 100);
        }
    };
