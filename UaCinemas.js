(function () {
            ...
            if ((e.type === 'complite' || e.type === 'complete') && e.data && e.data.movie) {
                ...
                var config = [
                    ...
                    },
                    {
                        ...
                        search: '/index.php?do=search&subaction=search&story=',
                        steps: [{ q: titleUa, checkYear: true }, { q: titleEn, checkYear: false }],
                ...
                if (rateLine.length) {
                    var torqCont = $('.quality-badges-container', rateLine); // сторонні плагіни
                    var tmdbRatings = $('.full-start-new__rating', rateLine); // стандартні рейтинги
                    var myCont;

                    var stack = $('.ua-plugins-stack', rateLine);
                    if (!stack.length) {
                        stack = $('<div class="ua-plugins-stack" style="display: flex; flex-direction: column; gap: 6px; align-items: flex-start; width: 100%;"></div>');
                        
                        if (torqCont.length) {
                            // Якщо є інший плагін, підлаштовуємось до нього
                            torqCont.before(stack);
                            stack.append(torqCont);
                        } else if (tmdbRatings.length) {
                            // Якщо немає плагінів, стаємо ПІСЛЯ останнього рейтингу (TMDB/IMDB)
                            tmdbRatings.last().after(stack);
                        } else {
                            // Фолбек, якщо нічого не знайдено
                            rateLine.append(stack);
                        }
                    }

                    myCont = $('.ua-sites-container', stack);
                    if (!myCont.length) {
                        myCont = $('<div class="ua-sites-container" style="display: flex; flex-wrap: wrap; gap: 6px; margin-left: 4px; margin-top: 2px;"></div>');
                        stack.append(myCont);
                    }

                    config.forEach(function(site) {
                        var btn = $('<div class="ua-btn-item selector" style="display: none; cursor: pointer; height: 1.55em; align-items: center; padding: 0 6px; box-sizing: border-box; background: rgba(255, 255, 255, 0.05); position: relative; border-radius: 8px;">' +
                            '<span style="color: #fff; font-weight: bold; font-size: 0.85em; white-space: nowrap; position: relative; z-index: 3;">' + site.title + '</span>' +
                        '</div>');
                                    ...
                                    success: function(html) {
                                        try {
                                            ...
                                            if (foundUrl) {
                                                ...
                                                btn.off('hover:enter click').on('hover:enter click', function(ev) {
                                                    ev.preventDefault();
                                                ...
                                                if (curr && (curr.name === 'full_start' || curr.name === 'full_descr')) {
                                                    curr.collection = render.find('.selector');
                                                }
                                            } else {
    ...
    var style = '<style id="ua-sites-style">' +
        ...
        '.ua-btn-item.focus { background: rgba(255, 255, 255, 0.2) !important; }' +
