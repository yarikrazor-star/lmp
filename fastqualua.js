(function () {
    ...
    // --- СТИЛІ (СТРОГО ОДИН РЯД + ПІДКЛАДКА) ---
    ...
    style.textContent = [
        ...
        '}',
        '.surs_quality .icon-main { width: 1.6em; height: 1.1em; margin-right: 7px; object-fit: contain; flex-shrink: 0; }',
        '.surs_quality .icon-stream { width: 1.2em; height: 1.2em; margin: 0 6px 0 10px; object-fit: contain; flex-shrink: 0; opacity: 0.8; }',
        '.surs_quality .quality-item { display: inline-flex !important; align-items: center !important; flex-wrap: nowrap !important; flex-shrink: 0 !important; }',
        '.surs_quality .seeds_info { margin-left: 4px; font-size: 0.8em; color: #00ff00 !important; font-weight: normal; }',
        '.surs_quality .pop-tag { text-transform: uppercase; font-size: 0.85em; letter-spacing: 0.5px; white-space: nowrap !important; }',
    ...
    // Запуск: спочатку намагаємось через подію готовності, якщо ні - через інтервал
    if (window.Lampa) {
        init();
    } else {
        $(document).on('lampa:ready', init);
        // Резервний варіант, якщо подія пролетіла повз
